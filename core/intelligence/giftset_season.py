...
#: ★2026-09-10 정정 — 리테일앤인사이트·제이티유통은 **오픈마켓**이다(사용자 확정).
#:   09-07 에 '상시 B2B 공급처'로 적었던 것을 뒤집는다.
...
OPEN_CHANNELS = [
    ...
    ("배민", ("우아한형제들",)),
    ("토스", ("토스뱅크",)),
    # ★상호명이 '오픈마켓'으로 시작하지 않고 **법인명으로 매출이 꽂히는** 오픈마켓 채널.
    #   플랫폼 정산이 아니라 거래처 명의로 들어와 B2B 로 오인되기 쉽다(2026-09-10 정정).
    ("리테일앤인사이트", ("리테일앤인사이트",)),
    ("제이티유통", ("제이티유통",)),
]
#: 위 중 표기 규칙(`오픈마켓~`)에 안 걸리지만 오픈마켓으로 집계하는 상호명 키워드.
OPEN_DIRECT = ("리테일앤인사이트", "제이티유통")
...
def _auto_group(s: str) -> str:
    ...
    if "자사몰" in s:
        return GRP_MALL
    if s.startswith("오픈마켓") or any(k in s for k in OPEN_DIRECT):
        return GRP_OPEN
    return ""
