...
# ── 소진 임박 경보(전역) ─────────────────────────────────────────────────────
# 탭을 열지 않아도 보이게 상단에 띄운다. 필터와 무관하게 항상 전건.
_URG = float(cfg.get("임박_일수", 3))
_urg_all = b[(b["세트재고"] > 0) & (b["일평균세트7"] > 0)
             & (b["소진예측일"] < _URG) & (b["상태"] != "판매중지")].sort_values("소진예측일")
# ★경보는 **조치가 남아 있는 것**만 센다. `추가입고없음` 은 발주할 수 없으므로
#   경보에 섞으면 급한 종수가 부풀고 매번 눈으로 걸러내야 한다(2026-09-09 사용자 지적).
#   버리지는 않고 아래 '재입고 불가' 한 줄로 남겨 노출 정리 대상으로 넘긴다.
_urg = _urg_all[~_urg_all["상태"].isin(_STATUS_DEAD)]
_urg_dead = _urg_all[_urg_all["상태"].isin(_STATUS_DEAD)]
if len(_urg):
    _head = " · ".join(f"{r['관리코드']} {r['상품명'][:12]}({r['소진예측일']:.1f}일)"
                       for _, r in _urg.head(6).iterrows())
    _more = f" 외 {len(_urg) - 6}종" if len(_urg) > 6 else ""
    st.error(f"🚨 **{_URG:.0f}일 내 소진 {len(_urg)}종** — {_head}{_more} "
             f"· 잔여 시즌 {k['잔여일']}일. 상세·조치는 **[재고 경보]** 탭 맨 위.")
elif len(_urg_dead):
    st.info(f"{_URG:.0f}일 내 소진이 임박한 {len(_urg_dead)}종은 모두 `추가입고없음` 입니다 "
            "— 발주 대상 없음. 소진 후 채널 노출만 정리하세요.")
...
with tabs[2]:
    ...
    # ── 소진 임박 ───────────────────────────────────────────────────────────
    if len(_urg) or len(_urg_dead):
        ui.section_head(f"{_URG:.0f}일 내 소진 — {len(_urg)}종", icon="🚨")
        st.caption(
            f"세트재고 ÷ 최근 7일 일평균 = **{_URG:.0f}일 미만**. 잔여 시즌 {k['잔여일']}일 중 "
            f"{_URG:.0f}일치만 남았다는 뜻입니다. **즉시 발주 검토**(시즌 생산 마감 여부 확인) — "
            "권장 수량은 아래 `권장 조정` 열.  \n"
            "※ 위 상태 필터와 무관하게 **항상 전건** 표시합니다. "
            "`추가입고없음`·`판매중지` 는 발주가 불가능하므로 이 표와 상단 경보에서 제외됩니다."
        )
        if len(_urg_dead):
            st.caption("🔁 **재입고 불가 " + f"{len(_urg_dead)}종**(소진 후 노출 정리 대상) — "
                       + " · ".join(f"{r['관리코드']} {str(r['상품명'])[:12]}"
                                    f"({r['소진예측일']:.1f}일·{r['상태']})"
                                    for _, r in _urg_dead.iterrows()))
        st.dataframe(
            _urg[["관리코드", "상품명", "상태", "촉진", "세트재고", "일평균세트7", "소진예측일",
                  "잔여시즌일", "시즌매출", "마진율", "주요채널계", "판정", "권장 조정"]]
            .rename(columns=gsn.DISPLAY_NAMES).style.format({
                "세트재고": "{:,.0f}", "일평균세트7": "{:,.1f}", "소진예측일": "{:,.1f}",
                "시즌매출": "{:,.0f}", "마진율": "{:.1%}", "주요채널계": "{:,.0f}"}),
            hide_index=True, width="stretch")
        st.download_button("⬇️ 소진 임박 XLSX", _to_xlsx(_urg_all, "소진임박"),
