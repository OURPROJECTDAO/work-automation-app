...
with tabs[2]:
    ...
    if len(_urg):
        ...
        _restock = _urg[~_urg["상태"].isin(_STATUS_DEAD)]
        _dead = _urg[_urg["상태"].isin(_STATUS_DEAD)]
        st.caption(
            f"세트재고 ÷ 최근 7일 일평균 = **{_URG:.0f}일 미만**. 잔여 시즌 {k['잔여일']}일 중 "
            f"{_URG:.0f}일치만 남았다는 뜻입니다.  \n"
            f"· **재입고 경로 있음 {len(_restock)}종** → 즉시 발주 검토(시즌 생산 마감 여부 확인). "
            f"권장 수량은 아래 `권장 조정` 열.  \n"
            f"· **`추가입고없음`·`판매중지` {len(_dead)}종** → 발주 불가. 소진 후 **저마진 채널부터 "
            "노출을 내려** 품절 노출을 줄입니다.  \n"
            "※ 이 표는 위 상태 필터와 무관하게 **항상 전건** 표시합니다."
        )
        st.dataframe(
